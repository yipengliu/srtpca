...
%
	...
	for m = 1:1:(2^L),
		y = cr(:,m).';
		w = CoarseMeyerCoeff(fft(y),L,nn,deg) ;
		crc(:,m) = w.';
	end
...
%
	...
	for j = L:1:(J-2),
		...
		for m = 1:1:(2^j),
			y = dr(:,m).';
			w = DetailMeyerCoeff(fft(y),j,nn,deg);
			drc(:,m) = w.';
		end
		...
		for m = 1:1:(2^j),
			y = dr(:,m).';
			w = CoarseMeyerCoeff(fft(y),j,nn,deg);
			drc(:,m) = w.';
		end
...
%  Compute vertical and diagonal contributions.
	...
	for m = 1:1:(2^(J-1)),
		y = dr(:,m).';
		w = CoarseMeyerCoeff(fft(y),(J-1),nn,deg);
		drc(:,m) = w.';
	end
